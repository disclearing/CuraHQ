...
public class DeathbanListener implements Listener {
    ...
    public DeathbanListener(Core plugin) {
        this.plugin = plugin;
    }
    ...
    public void onPlayerLogin(PlayerLoginEvent event) {
        ...
        if (player.hasPermission(DeathbanListener.DEATH_BAN_BYPASS_PERMISSION) || (ConfigUtil.KIT_MAP) == true) {
            return;
        }
        ...
        if (lastAttemptedJoinMillis != this.lastAttemptedJoinMap.getNoEntryValue() && lastAttemptedJoinMillis - millis < DeathbanListener.LIFE_USE_DELAY_MILLIS) {
            ...
            new DelayedMessageRunnable(plugin, player, ChatColor.YELLOW + "You have used a life for entry. You now have " + ChatColor.WHITE + lives + ChatColor.YELLOW + " lives.").runTask(plugin);
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if ((player.hasPermission(DeathbanListener.DEATH_BAN_BYPASS_PERMISSION)) || (player.isOp() || (ConfigUtil.KIT_MAP) == true)) {
        	return;
        }
    ...
    public void onPlayerRequestRespawn(PlayerRespawnEvent event) {
        ...
        if (deathban != null && deathban.getRemaining() > 0L) {
            if ((player.hasPermission(DeathbanListener.DEATH_BAN_BYPASS_PERMISSION) || (ConfigUtil.KIT_MAP) == true)) {
                ...
                //new DelayedMessageRunnable(plugin, player, ChatColor.RED + "You would be death-banned for " + deathban.getReason() + ChatColor.RED + ", but you have access to bypass.").runTask(this.plugin);
