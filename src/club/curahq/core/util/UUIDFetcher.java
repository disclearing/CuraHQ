...
public class UUIDFetcher implements Callable<Map<String, UUID>> {
    ...
    public UUIDFetcher(final List<String> names, final boolean rateLimiting) {
        ...
        this.names = (List<String>) ImmutableList.copyOf((Collection<String>) names);
    ...
    public Map<String, UUID> call() throws Exception {
        ...
        for (int requests = (int) Math.ceil(this.names.size() / 100.0), i = 0; i < requests; ++i) {
            ...
            final String body = JSONArray.toJSONString((List<String>) this.names.subList(i * 100, Math.min((i + 1) * 100, this.names.size())));
